package cn.net.leadu.util.esign;
...
public class EviDocHelper {
	 ...
	 *
	 * @param personalInfo
	 * @param flag false：存证确认函、true：存证合同
	 * @return
	 */
	public static boolean eviDoc(PersonalInfo personalInfo, boolean flag) {
		String pdfUrl;
		if(flag){
		...
		String[] arr = pdfUrl.split("http://222.73.56.12:8588/files/download/signedPdf/"); //http://192.168.1.115:8588/files/download/signedPdf/
		if(arr.length != 2){
			return false;
		}
		...
		// 用户获取文档保全Url和存证编号
		Map<String, String> eviMap = getEviUrlAndEvId(personalInfo, flag);
	 ...
	 * @return
	 */
	public static Map<String, String> getEviUrlAndEvId(PersonalInfo personalInfo, boolean flag) {
		Map<String, String> hashMap = null;
		JSONObject jsonObj = eviRequestByPost(personalInfo, flag);
	 ...
	 * @return
	 */
	public static JSONObject eviRequestByPost(PersonalInfo personalInfo, boolean flag) {
		StringBuffer strBuffer = null;
		try {
			// 建立连接
			URL url = new URL(eviUrl);
			HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
			initPostHeaders(httpURLConnection, personalInfo, flag);
			// 连接会话
			httpURLConnection.connect();
			// 建立输入流，向指向的URL传入参数
			DataOutputStream dos = new DataOutputStream(httpURLConnection.getOutputStream());
			// 设置请求参数
			dos.write(setPostJSONStr(personalInfo, flag).getBytes("UTF-8"));
			System.out.println("body = " + setPostJSONStr(personalInfo, flag));
			dos.flush();
			dos.close();
			// 获得响应状态
	 ...
	 * @return
	 */
	public static void initPostHeaders(HttpURLConnection httpURLConnection, PersonalInfo personalInfo, boolean flag) {
		// 通过HmacSHA256算法获取签名信息，用以验签
		String signature = AlgorithmHelper.getXtimevaleSignature(setPostJSONStr(personalInfo, flag), projectSecret, algorithm, encoding);
		// 设置Headers参数
		try {
			httpURLConnection.setDoOutput(true); // 需要输出
			httpURLConnection.setDoInput(true); // 需要输入
	 ...
	 * @return
	 */
	public static String setPostJSONStr(PersonalInfo personalInfo, boolean flag) {
		...
		eviObj.element("eSignIds", setEsignIds(flag ? personalInfo.getSignServiceId() : personalInfo.getSignConfirmationServiceId()));
